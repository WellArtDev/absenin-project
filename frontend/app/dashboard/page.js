...
export default function DashboardPage() {
  ...
  // Stable change handler for employee form
  const handleEfChange = useCallback((field, value) => {
  ...
  const startEdit = (emp) => {
    setEf({
      name: emp.name || '', phone_number: emp.phone_number || '',
      employee_code: emp.employee_code || '', email: emp.email || '',
  ...
  // Stable change handler for settings form — avoids stale closure bugs
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-50">{analytics.recentAttendance?.length > 0 ? analytics.recentAttendance.map((a, i) => (
                  <div key={i} className="px-6 py-3 flex items-center justify-between hover:bg-gray-50">
                    <div className="flex items-center gap-3"><Av name={a.employee_name} /><div><p className="text-sm font-medium">{a.employee_name}{a.selfie_checkin_url && ' 📸'}{a.location_name && ' 📍'}</p><p className="text-xs text-gray-500">{a.division_name || '-'}</p></div></div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-50">{analytics.notCheckedIn?.length > 0 ? analytics.notCheckedIn.map((e, i) => (
                  <div key={i} className="px-6 py-3 flex items-center justify-between hover:bg-gray-50">
                    <div className="flex items-center gap-3"><Av name={e.name} color="bg-red-100 text-red-600" /><div><p className="text-sm font-medium">{e.name}</p><p className="text-xs text-gray-500">{e.division_name || '-'}</p></div></div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6 animate-slide-up">
                ...
                <form onSubmit={addEmp}>
                  ...
                  <div className="grid md:grid-cols-3 gap-4 mb-6">
                    <InputField label="Nama" required type="text" value={ef.name} onChange={e => handleEfChange('name', e.target.value)} placeholder="Nama lengkap" />
                    <InputField label="No. WhatsApp" required type="text" value={ef.phone_number} onChange={e => handleEfChange('phone_number', e.target.value)} placeholder="08123456789" />
                    <InputField label="Email" type="email" value={ef.email} onChange={e => handleEfChange('email', e.target.value)} placeholder="email@company.com" />
                    <InputField label="Kode Karyawan" type="text" value={ef.employee_code} onChange={e => handleEfChange('employee_code', e.target.value)} placeholder="Auto jika kosong" />
                    <SelectField label="Divisi" value={ef.division_id} onChange={e => handleEfChange('division_id', e.target.value)}>
                      <option value="">-- Pilih Divisi --</option>
                      {divisions.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                    </SelectField>
                    <SelectField label="Jabatan" value={ef.position_id} onChange={e => handleEfChange('position_id', e.target.value)}>
                  ...
                  <div className="grid md:grid-cols-3 gap-4 mb-6">
                    <SelectField label="Status Karyawan" value={ef.employment_status} onChange={e => handleEfChange('employment_status', e.target.value)}>
                    ...
                    <InputField label="Tanggal Mulai Kerja" type="date" value={ef.start_date} onChange={e => handleEfChange('start_date', e.target.value)} />
                    <InputField label="Gaji Pokok" type="number" value={ef.base_salary} onChange={e => handleEfChange('base_salary', e.target.value)} placeholder="0" />
                    <InputField label="Sisa Cuti" type="number" value={ef.leave_balance} onChange={e => handleEfChange('leave_balance', e.target.value)} />
                    <div className="flex items-center gap-3 pt-6">
                      <input type="checkbox" id="radius_lock" checked={ef.radius_lock_enabled} onChange={e => handleEfChange('radius_lock_enabled', e.target.checked)} className="w-4 h-4 text-brand-500 rounded" />
                  ...
                  <div className="grid md:grid-cols-3 gap-4 mb-6">
                    <InputField label="No. KTP" type="text" value={ef.ktp_number} onChange={e => handleEfChange('ktp_number', e.target.value)} placeholder="16 digit" />
                    <InputField label="NPWP" type="text" value={ef.npwp_number} onChange={e => handleEfChange('npwp_number', e.target.value)} />
                    <InputField label="Tanggal Lahir" type="date" value={ef.birth_date} onChange={e => handleEfChange('birth_date', e.target.value)} />
                    <InputField label="Tempat Lahir" type="text" value={ef.birth_place} onChange={e => handleEfChange('birth_place', e.target.value)} />
                    <SelectField label="Jenis Kelamin" value={ef.gender} onChange={e => handleEfChange('gender', e.target.value)}>
                      <option value="">-- Pilih --</option><option value="L">Laki-laki</option><option value="P">Perempuan</option>
                    </SelectField>
                    <InputField label="Kontak Darurat" type="text" value={ef.emergency_contact} onChange={e => handleEfChange('emergency_contact', e.target.value)} />
                    <InputField label="HP Darurat" type="text" value={ef.emergency_phone} onChange={e => handleEfChange('emergency_phone', e.target.value)} />
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">Alamat</label>
                      <textarea value={ef.address} onChange={e => handleEfChange('address', e.target.value)} rows={2} className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-500/20 text-sm" />
                    </div>
                  </div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-50">{filteredEmployees.map(emp => (
                        <tr key={emp.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4"><div className="flex items-center gap-3"><Av name={emp.name} /><div><span className="text-sm font-medium">{emp.name}</span></div></div></td>
              ...
              {attLoading ? <Spinner /> : attData.length > 0 ? (
                <div className="overflow-x-auto"><table className="w-full"><thead><tr className="bg-gray-50 border-b">
                  ...
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="px-6 py-4"><div className="flex items-center gap-3"><Av name={a.employee_name} /><div><p className="text-sm font-medium">{a.employee_name}</p><p className="text-xs text-gray-500">{a.employee_code || ''} {a.division_name || ''}</p></div></div></td>
        ...
        {tab === 'overtime' && (
          <div className="space-y-6 animate-fade-in">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <select value={rMonth} onChange={e => setRMonth(parseInt(e.target.value))} className="px-3 py-2 rounded-xl border border-gray-200 text-sm bg-white">{mons.slice(1).map((m, i) => <option key={i} value={i + 1}>{m}</option>)}</select>
                <select value={rYear} onChange={e => setRYear(parseInt(e.target.value))} className="px-3 py-2 rounded-xl border border-gray-200 text-sm bg-white">{[2024, 2025, 2026].map(y => <option key={y} value={y}>{y}</option>)}</select>
                <button onClick={loadOT} className="bg-gray-100 text-gray-700 px-4 py-2 rounded-xl text-sm hover:bg-gray-200">🔄</button>
              </div>
            </div>
            {otLoading ? <Spinner /> : (
              <>
                ...
                {otSummary.filter(e => (parseInt(e.overtime_days) || 0) > 0).length > 0 && (
                  <div className="bg-white rounded-2xl border overflow-hidden">
                    ...
                    <div className="overflow-x-auto"><table className="w-full"><thead><tr className="bg-gray-50 border-b">
                      ...
                      <tr key={i} className="hover:bg-gray-50">
                        <td className="px-6 py-4"><div className="flex items-center gap-3"><Av name={e.name} /><div><span className="text-sm font-medium">{e.name}</span><p className="text-xs text-gray-500">{e.division_name || '-'}</p></div></div></td>
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <select value={rMonth} onChange={e => setRMonth(parseInt(e.target.value))} className="px-3 py-2 rounded-xl border border-gray-200 text-sm bg-white">{mons.slice(1).map((m, i) => <option key={i} value={i + 1}>{m}</option>)}</select>
                <select value={rYear} onChange={e => setRYear(parseInt(e.target.value))} className="px-3 py-2 rounded-xl border border-gray-200 text-sm bg-white">{[2024, 2025, 2026].map(y => <option key={y} value={y}>{y}</option>)}</select>
                <button onClick={doExport} className="bg-brand-500 text-white px-5 py-2 rounded-xl text-sm font-semibold hover:bg-brand-600 shadow-sm">📥 Export CSV</button>
              </div>
            </div>
            <div className="bg-white rounded-2xl border overflow-hidden">
              <div className="px-6 py-4 border-b"><h3 className="font-semibold">Ringkasan — {mons[rMonth]} {rYear}</h3></div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border p-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <InputField label="Nama Perusahaan" type="text" value={settingsForm.company_name || ''} onChange={e => handleSettingsChange('company_name', e.target.value)} />
                    <InputField label="Email" type="email" value={settingsForm.company_email || ''} onChange={e => handleSettingsChange('company_email', e.target.value)} />
                    <InputField label="Telepon" type="text" value={settingsForm.company_phone || ''} onChange={e => handleSettingsChange('company_phone', e.target.value)} />
                    <InputField label="Logo URL" type="text" value={settingsForm.logo_url || ''} onChange={e => handleSettingsChange('logo_url', e.target.value)} />
                    <div className="md:col-span-2"><label className="block text-sm font-medium text-gray-700 mb-1">Alamat</label><textarea value={settingsForm.company_address || ''} onChange={e => handleSettingsChange('company_address', e.target.value)} rows={2} className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 text-sm transition-colors" /></div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border p-6">
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    <InputField label="Jam Masuk" type="time" value={settingsForm.work_start || '08:00'} onChange={e => handleSettingsChange('work_start', e.target.value)} />
                    <InputField label="Jam Pulang" type="time" value={settingsForm.work_end || '17:00'} onChange={e => handleSettingsChange('work_end', e.target.value)} />
                    <InputField label="Toleransi Telat (menit)" type="number" value={settingsForm.late_tolerance_minutes || 15} onChange={e => handleSettingsChange('late_tolerance_minutes', parseInt(e.target.value))} />
                  </div>
                  <div className="flex flex-wrap gap-6 mt-4">
                    <label className="flex items-center gap-2"><input type="checkbox" checked={settingsForm.require_selfie || false} onChange={e => handleSettingsChange('require_selfie', e.target.checked)} className="w-4 h-4 text-brand-500 rounded" /><span className="text-sm">📸 Wajib Selfie</span></label>
                    <label className="flex items-center gap-2"><input type="checkbox" checked={settingsForm.require_location || false} onChange={e => handleSettingsChange('require_location', e.target.checked)} className="w-4 h-4 text-brand-500 rounded" /><span className="text-sm">📍 Wajib Lokasi</span></label>
                    <label className="flex items-center gap-2"><input type="checkbox" checked={settingsForm.overtime_enabled || false} onChange={e => handleSettingsChange('overtime_enabled', e.target.checked)} className="w-4 h-4 text-brand-500 rounded" /><span className="text-sm">🕐 Lembur Aktif</span></label>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border p-6">
                  ...
                  <div className="grid md:grid-cols-3 gap-4 mb-4">
                    <InputField label="Latitude" type="text" value={settingsForm.office_latitude || ''} onChange={e => handleSettingsChange('office_latitude', e.target.value)} placeholder="-6.2088" />
                    <InputField label="Longitude" type="text" value={settingsForm.office_longitude || ''} onChange={e => handleSettingsChange('office_longitude', e.target.value)} placeholder="106.8456" />
                    <InputField label="Radius (meter)" type="number" value={settingsForm.allowed_radius_meters || 500} onChange={e => handleSettingsChange('allowed_radius_meters', parseInt(e.target.value))} />
                  </div>
                  <div className="flex items-center gap-4 mb-4">
                    <label className="flex items-center gap-2"><input type="checkbox" checked={settingsForm.radius_lock_enabled || false} onChange={e => handleSettingsChange('radius_lock_enabled', e.target.checked)} className="w-4 h-4 text-brand-500 rounded" /><span className="text-sm font-medium">🔒 Aktifkan Radius Lock Global</span></label>
                  </div>
                  <div className="md:col-span-3"><label className="block text-sm font-medium text-gray-700 mb-1">Alamat Kantor</label><textarea value={settingsForm.office_address || ''} onChange={e => handleSettingsChange('office_address', e.target.value)} rows={2} className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 text-sm transition-colors" placeholder="Jl. Sudirman No. 1, Jakarta" /></div>
                ...
                <div className="bg-white rounded-2xl border p-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <InputField label="Fonnte API URL" type="text" value={settingsForm.wa_api_url || ''} onChange={e => handleSettingsChange('wa_api_url', e.target.value)} placeholder="https://api.fonnte.com/send" />
                    <InputField label="Fonnte Token" type="text" value={settingsForm.wa_api_token || ''} onChange={e => handleSettingsChange('wa_api_token', e.target.value)} placeholder="Token dari Fonnte" />
                    <InputField label="Nomor WA Device" type="text" value={settingsForm.wa_device_number || ''} onChange={e => handleSettingsChange('wa_device_number', e.target.value)} placeholder="628xxx (nomor yang terhubung di Fonnte)" />
                  </div>
