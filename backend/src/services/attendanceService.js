...
class AttendanceService {
  // forcedCompanyId: when known from wa_device_number, scope lookup to that company only
  static async processCheckIn(phoneNumber, messageText, location = null, imageData = null, forcedCompanyId = null) {
    try {
      const cleanPhone = phoneNumber.replace(/[^0-9]/g, '');

      const buildEmpQuery = (extraWhere) => `
        ...
        WHERE e.phone_number=$1 AND e.is_active=true ${extraWhere}`;

      const companyWhere = forcedCompanyId ? `AND e.company_id=${parseInt(forcedCompanyId)}` : '';

      let empResult = await query(buildEmpQuery(companyWhere), [cleanPhone]);

      if (empResult.rows.length === 0) {
        // Normalise phone format and retry
        const altPhone = cleanPhone.startsWith('62') ? '0' + cleanPhone.substring(2) : '62' + cleanPhone;
        empResult = await query(buildEmpQuery(companyWhere), [altPhone]);
        if (empResult.rows.length === 0) {
          return { success: false, companyId: forcedCompanyId, reply: '❌ Nomor Anda belum terdaftar di sistem.\nHubungi admin perusahaan Anda.' };
        }
      }
      ...
      if (comp.rows.length === 0 || !comp.rows[0].is_active) {
        return { success: false, companyId: employee.company_id, reply: '❌ Perusahaan Anda tidak aktif. Hubungi admin.' };
      }
      ...
      switch (true) {
        case ['HADIR', 'MASUK', 'CHECKIN', 'IN', 'ABSEN'].includes(cmd):
          result = await this.checkIn(employee, location, imageData); break;
        case ['PULANG', 'KELUAR', 'CHECKOUT', 'OUT'].includes(cmd):
          result = await this.checkOut(employee, location, imageData); break;
        case ['STATUS', 'CEK', 'INFO'].includes(cmd):
          result = await this.getStatus(employee); break;
        case cmd === 'IZIN':
          result = await this.markStatus(employee, 'IZIN'); break;
        case cmd === 'SAKIT':
          result = await this.markStatus(employee, 'SAKIT'); break;
        case cmd.startsWith('LEMBUR'):
          result = await this.requestOvertime(employee, messageText); break;
        case ['SELESAI LEMBUR', 'STOP LEMBUR', 'END LEMBUR', 'DONE LEMBUR'].includes(cmd):
          result = await this.finishOvertime(employee); break;
        case ['REKAP', 'REKAP LEMBUR'].includes(cmd):
          result = await this.getOvertimeSummary(employee); break;
        default:
          result = this.getHelp(employee);
      }

      // Always include companyId so webhook knows which WA config to use for reply
      return { ...result, companyId: employee.company_id };
    } catch (error) {
      console.error('❌ processCheckIn error:', error);
      return { success: false, companyId: forcedCompanyId, reply: '⚠️ Terjadi kesalahan sistem. Coba lagi nanti.' };
    }
  }
